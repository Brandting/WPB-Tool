// ============================================================================
// BAUZEITPLAN — Gantt-Chart, Vorgänge, Abhängigkeiten, MS-Project Import/Export.
  ...
  return `
    ...
    ${tasks.map(t=>{
      ...
      return `<tr>
        ...
        <td class="actions-cell">${actionButtons(t.id,{edit:'editScheduleTask',del:'deleteScheduleTask'})}</td>
      </tr>`;
    }).join('')}
    </tbody></table></div>`}
  `;
...
async function deleteScheduleTask(id){
  ...
  if(!confirm(`Vorgang „${t?.name||'?'}" wirklich löschen?`)) return;
